...
GROUPS = {
    ...
    # [임시] 일본 티니핑 표기 후보 비교 — 어느 표기로 실제 검색되는지 확인용.
    # 구글은 한 그룹의 키워드들을 함께 정규화하므로 크기 비교가 된다.
    # 정식명 キャッチ!ティニピン(2024~ TOKYO MX), 극장판 愛のハチュピン, 캐릭터 ハチュピン.
    # 기존 설정값 ティーニーピン(장음 표기)은 위키에서도 안 잡혀 오타로 의심된다.
    # 승자 확인 후 이 그룹은 지우고 '티니핑 국가별' 의 일본 키워드만 교체할 것.
    # 한 그룹에 묶으면 검색량 미달 키워드 하나가 드롭되며 그룹 전체가 실패한다
    # (구글이 그 컬럼을 아예 빼버림). geos 로 따로 조회해 실패를 격리한다.
    "일본 티니핑 키워드": {
        "geos": [
            {"label": "ティニピン",        "geo": "JP", "kw": "ティニピン"},
            {"label": "キャッチティニピン", "geo": "JP", "kw": "キャッチ!ティニピン"},
            {"label": "ハチュピン",        "geo": "JP", "kw": "ハチュピン"},
            {"label": "愛のハチュピン",     "geo": "JP", "kw": "愛のハチュピン"},
            {"label": "ティーニーピン",     "geo": "JP", "kw": "ティーニーピン"},
        ],
        "freq": "week",
    },
    # 메탈카드봇 수출국 추이 — 나라마다 현지 표기로 따로 조회.
    # 러시아는 라틴 'Metal Cardbot' 으로는 거의 안 잡힌다. 현지 정식명이 키릴
    # «Метал Кард Бот» (2025-01 러시아어 더빙, Карусель·ivi 방영)이라 그걸 써야 한다.
    # 러시아 검색은 얀덱스 점유가 더 높지만 Wordstat 은 계정이 있어야 해서 구글로 본다.
    "메탈카드봇 국가별": {
        "geos": [
