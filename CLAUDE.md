...
## 탑툰챗 — DAU 가 아니라 '대화수 증분' 이다 (2026-08-31)

탑코미디어의 AI 챗봇(탑툰챗)은 **DAU 를 공개하지 않는다.** 사이트가 주는 건 캐릭터별
**누적** 대화수·조회수·좋아요뿐이다. 그래서 `fetch_toptoonchat.py` 가 매일 한 번 찍고,
화면에서 **일별 증분**(= 그날 새로 시작된 대화 수)으로 바꿔 그린다 — 대리지표이지 DAU 가 아니다.
**이걸 'DAU' 라고 부르지 말 것.**

- 수집: 홈 HTML 안 Next.js flight 페이로드에 캐릭터 배열이 그대로 있다(브라우저 불필요).
  한 번 요청에 KR 51명·JP 45명(홈 노출분. 전체 캐릭터 목록이 아니다).
- ⚠ 조회수 필드는 **`views`** 다. `viewCount` 는 일부(멀티 작품) 객체에만 있어서
  그것만 보면 조회수가 통째로 0 이 된다(도입 때 그랬다). 둘 다 보되 `views` 우선.
- 같은 캐릭터가 인기/신규/트렌드 섹션에 중복 등장하므로 **id 로 합친다.**
- KR·JP 를 따로 쌓는다 — 2026년 일본(5월)·북미(7월) 진출이라 지역별 성장이 핵심.
  북미는 전용 도메인을 아직 못 찾았다(`chat.toptoon.us`·`/en` 둘 다 실패).
- **증분은 점이 2개 이상 쌓여야 나온다.** 수집 이튿날부터 그래프가 생긴다.
- 화면은 트렌드 비교 탭에 `TREND_STOCK["탑코미디어"]` 로 붙는데, **트렌드 탭 종목 버튼은
  `DATA`(커버리지)에서 만들어진다.** 탑코미디어를 `DATA` 에 넣기 전까지는 화면에 안 뜬다
  (데이터는 그동안에도 계속 쌓인다).
