...
**컨센(실적 전환) 확인은 하루 3회다** — `refresh_live.py` 의 deep 수집이 '그날 첫 회차(08시)
+ 16시 + 18시'. 실적 발표는 장중~마감 후에 뜨는데 첫 회차에만 받으면 그날 발표분을 다음 날
아침에야 '실적'으로 보게 된다(2026-08-21 실측: 8/20 발표 4건이 8/21 08:06 에 몰려서 알림).
어느 슬롯까지 받았는지는 `LIVE.deepSlot`(`YYYY-MM-DD#시`)에 남겨 같은 슬롯 중복을 막는다.
`notify.py` 는 매 회차 돌지만 실제 감지는 이 deep 회차 뒤에만 된다.
