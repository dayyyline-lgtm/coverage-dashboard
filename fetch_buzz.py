...
LISTED = [
    ...
    # ── 커버리지 밖 상장사 — 섹터 온도·경쟁 지형 ─────────────────────
    # 블루 아카이브: 넥슨게임즈가 개발(코스닥 상장) · 넥슨이 퍼블리싱(도쿄).
    (["블루 아카이브", "블루아카이브"],
        [("넥슨게임즈", "225570", "KOSDAQ", 0), ("넥슨", "3659", "TSE", 0)]),
    (["던파", "던전앤파이터", "메이플", "마비노기", "카트라이더", "서든어택", "FC 온라인", "페리온"],
                                        [("넥슨", "3659", "TSE", 0)]),
    ...
    # 해외 상장사 — 국내 커버리지엔 없지만 IP 주인이 분명한 것들
    (["리그 오브 레전드", "발로란트", "라이엇", "소환사의 협곡", "제트(발로란트",
      "클래시 로얄", "리프트바운드"],          [("텐센트", "0700", "HKEX", 0)]),
    (["오버워치", "디아블로", "월드 오브 워크래프트", "스타크래프트", "콜 오브 듀티"],
                                        [("마이크로소프트", "MSFT", "NASDAQ", 0)]),
    (["포켓몬", "닌텐도", "젤다", "마리오"],   [("닌텐도", "7974", "TSE", 0)]),
    (["우마무스메", "그랑블루", "섀도우버스", "마나리아"],
                                        [("사이버에이전트", "4751", "TSE", 0)]),
    (["유희왕", "이풋볼", "위닝일레븐", "실황파워풀"], [("코나미", "9766", "TSE", 0)]),
    (["몬스터헌터", "스트리트파이터", "바이오하자드"], [("캡콤", "9697", "TSE", 0)]),
    (["건담", "아이돌마스터", "밀리시타", "엘든링"], [("반다이남코", "7832", "TSE", 0)]),
    (["헬다이버즈", "고스트 오브", "라스트 오브 어스"], [("소니", "6758", "TSE", 0)]),
    (["프롬 소프트웨어", "다크소울", "세키로"],  [("카도카와", "9468", "TSE", 0)]),
    (["홀로라이브"],                        [("커버", "5253", "TSE", 0)]),
    (["뱅드림", "BanG Dream"],             [("부시로드", "7803", "TSE", 0)]),
    (["로블록스", "Roblox"],                [("로블록스", "RBLX", "NYSE", 0)]),
    (["쿠팡"],                            [("쿠팡", "CPNG", "NYSE", 0)]),
    (["갤럭시 S", "갤럭시 Z", "갤럭시워치"],   [("삼성전자", "005930", "KOSPI", 0)]),
    (["마운자로", "젭바운드"],                [("일라이 릴리", "LLY", "NYSE", 0)]),
    (["위고비", "오젬픽"],                   [("노보 노디스크", "NVO", "NYSE", 0)]),
    (["맨체스터 유나이티드"],                 [("맨체스터 유나이티드", "MANU", "NYSE", 0)]),
    (["F1(포뮬러", "포뮬러 원"],             [("리버티 미디어", "FWONK", "NASDAQ", 0)]),
]
