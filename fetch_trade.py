...
REGION_ITEMS = [
    ...
    # 빙과 — 롯데웰푸드·해태도 있는 품목이라 전국 210500 은 빙그레 프록시가 될 수 없다.
    # 그런데 실측하니 **남양주 한 곳이 85.9%** 였다(26.1~6 누적 $50.1M/$58.3M · 2위 화순 2.8%).
    # 빙그레 본사·도농공장이 남양주다. 논산·김해·광주 공장 몫도 남양주로 신고되는 것으로 보인다.
    # 2026-04-01 해태아이스크림 흡수합병으로 해태 물량까지 빙그레 것이 됐다(존속법인 빙그레).
    {"label": "빙과(남양주)", "hs": ["210500"], "sidoCd": "41", "sgg": "남양주",
     "note": "빙그레 남양주(본사·도농공장) · 아이스크림·빙과류(210500) 제조지 기준 = 메로나·투게더 프록시. "
             "전국 210500 수출의 85.9%가 남양주다(26.1~6 누적 $50.1M/$58.3M · 2위 전남 화순 2.8%). "
             "26-04-01 해태아이스크림 흡수합병 이후는 해태 물량도 여기 포함."},
    {"label": "바나나맛우유(남양주)", "hs": ["040299"], "sidoCd": "41", "sgg": "남양주",
     "note": "빙그레 남양주공장 · 기타 가공유류(040299, 관세청 품목명은 '기타') 제조지 기준 = 바나나맛우유 프록시. "
             "전국 040299 수출의 ~90%가 남양주다(26.1~6 누적 $15.4M/$17.2M · 나머지는 충남 아산·부산 서구 등 소량)."},
    # 제닉 — 마스크팩 ODM 인데 공장(논산)으로는 안 잡힌다. 실측 논산 330790 은 25~26 누적 $1.7M 뿐이다.
    # 물량은 **고객사 본사 소재지**로 신고된다 — 주 고객 바이오던스(콜라겐 마스크)가 강남구다.
    # 강남구 330790 은 전국 시군구 1위($304M · 2위 송파 $190M · 경기 1위 파주 $125M).
    # ⚠ 강남에는 다른 화장품 본사도 많아(에이피알·마녀공장 등) 순수 제닉 몫이 아니다 — '강남 발 마스크팩'
    #   전체를 바이오던스·제닉 프록시로 읽는 것. 시군구 API 는 6자리만 받아 3307904000 으로 못 좁힌다(330790 = 마스크팩 전체 기준).
    # 26.04 $41.3M 이 단독 급등(전후 $19·15M) — 제닉 2Q26 매출 492억(1Q 294억)과 방향이 같다.
    {"label": "마스크팩 전체(강남)", "hs": ["330790"], "sidoCd": "11", "sgg": "강남",
     "note": "제닉 프록시 — 바이오던스(강남구 본사) 등 강남 발 마스크팩류(330790) 수출. 공장(논산)이 아니라 고객사 본사로 신고된다. "
             "강남구가 전국 시군구 1위(25~26 누적 $304M · 2위 송파 $190M). 타 강남 본사 브랜드도 섞이므로 순수 제닉 몫은 아니다."},
]
# 지역 프록시 → 종목 · 같은 HS 의 전국 계열(비중 계산용). 화면(트렌드 탭)이 이걸로 종목에 붙인다.
#   여기 한 곳에만 둔다 — 화면에 사전을 또 두면 갈라진다.
REGION_META = {
    "리쥬란(강릉 기타화장품)": {"stock": "파마리서치", "natl": None},
    "창상피복재(안성)":         {"stock": "티앤엘",     "natl": None},
    "빙과(남양주)":             {"stock": "빙그레",     "natl": "빙과"},
    "바나나맛우유(남양주)":      {"stock": "빙그레",     "natl": "가공유"},
    "마스크팩 전체(강남)":       {"stock": "제닉",       "natl": "마스크팩 전체"},
}
...
# ══════════════════════════════════════════════════════════════════
# HS 부호 지도 (2026-08-01 관세청 statKor 로 실측 · 26.06 수출 백만달러)
#
# ⚠️ 부호는 반드시 '품목명(statKor)'으로 고를 것. 금액이 맞는지로 고르면 틀린다.
#    실제로 세 번 틀렸다 — 증권사 표와 숫자가 맞아떨어져서 맞다고 믿었는데,
#    그건 그쪽이 상위 부호를 그 이름으로 부르고 있었을 뿐이었다.
#      기초    330499(1,009) 로 잡음 → 실제 3304991000(555)
#      마스크팩 330790(117) 로 잡음 → 실제 3307904000(67)
#      라면    190230(175) 로 잡음  → 실제 1902301010(165)
#    확인법: nitemtrade 를 hsSgn=<6자리> 로 부르면 10자리와 statKor 가 같이 온다.
#
# 3303  향수·화장수 ....................... 9.5
#   330300  향수와 화장수
# 3304  화장품 ......................... 1,088.5
#   330499  기타 ........................ 1,009.4
#     3304991000  기초화장용 제품류 ......... 554.7   ← '기초'
#     3304999000  기타 ................... 376.7
#     3304992000  메이크업용 제품류 .......... 75.7   ← '색조'에 포함
#     3304993000  어린이용 제품류 ............. 2.3
#   330410  입술화장용 (립스틱 3304101000) .. 50.5   ← '색조'
#   330420  눈화장용 (아이섀도 3304201000) .. 20.4   ← '색조'
#   330491  가루 ......................... 5.6   ← '색조'
#   330430  매니큐어·페디큐어 ............... 2.7
# 3305  두발용 .......................... 51.6
#   330590  기타 ........................ 34.0
#     3305909000 기타 29.2 · 3305901000 헤어린스 3.4 · 3305902000 헤어크림 1.4
#   330510  샴푸 ........................ 16.9
#   330520 퍼머넌트 0.5 · 330530 헤어래커 0.1
# 3307  조제향료·화장품류(따로 분류 안 된 것) . 120.7
#   330790  기타 ....................... 117.1
#     3307904000  마스크 팩 .............. 67.0   ← '마스크팩'
#     3307909000  기타 ................... 49.7
#     3307903000 콘택트렌즈액 0.4 · 3307901000 제모제 0.0
#   330749 방향제 2.1 · 330720 탈취제 1.1 · 330730 목욕용 0.4 · 330710 면도용 0.0
# 1902  파스타 ......................... 188.4
#   190230  그 밖의 파스타 .............. 174.9
#     1902301010  라면 .................. 165.2   ← '라면'
#     1902301090 기타 6.3 · 1902309000 기타 3.4
#   190220  속을 채운 파스타(만두) ......... 7.6
# 2105  아이스크림·빙과류 ................. 75.2   ← '빙과' (26.1~6 누적)
#   210500  아이스크림과 그 밖의 빙과류 ..... 75.2   6자리 밑에 세분 없음
#     시군구: 경기 남양주 85.9% · 전남 화순 2.8% · 충북 음성 2.3% (빙그레 프록시로 유효)
# 0402  농축·가당 우유·크림
#   040299  기타 ......................... 17.1   ← '가공유' (26.1~6 누적)
#     0402999000  기타 .................. 17.1   ← 바나나맛우유가 여기
#     0402991000  가당연유 ............... 0.005
# 3005  창상피복재 ....................... 17.1
#   300590 기타 10.4 · 300510 접착성 피복재 6.7
# ══════════════════════════════════════════════════════════════════
...
ITEMS = [
    # ── 빙그레 (2026-09-08 추가) ─────────────────────────────────
    # 지역 프록시(남양주)는 '얼마나 나가나'만 준다. **어느 나라로 나가나**는 국가별이라야 보인다
    # — 사용자 관심이 미국·기타 지역의 메로나·바나나맛우유라 국가별을 같이 받는다.
    # 210500 은 6자리 밑에 세분이 없다(관세청 statKor: '아이스크림과 그 밖의 빙과류') — 26.1~6 $75.2M.
    {"hs": "210500", "label": "빙과", "note": "아이스크림·빙과류 (210500) — 빙그레(해태 합병)·롯데웰푸드 등 합산"},
    # 가공유는 0402999000('기타') 하나다. 040299 6자리로 부르면 가당연유(0402991000)가 섞이는데
    # 금액이 0.03% 라 실무상 차이는 없지만, 부호는 품목명으로 고른다는 규칙대로 10자리를 쓴다.
    {"hs": "0402999000", "label": "가공유", "note": "기타 가공유류 (0402999000) — 바나나맛우유 등"},
    ...
    # 마스크팩은 3307904000 하나다(관세청 품목명 그대로 '마스크 팩').
    # 26.06 구성: 3307904000 마스크팩 67.0 · 3307909000 기타 49.7
    #             3307903000 콘택트렌즈액 0.4 · 3307901000 제모제 0.0
    # 증권사 표의 '마스크팩' 117 은 330790 전체(마스크팩+기타)를 그렇게 부른 것이라
    # 숫자가 맞는다고 따라가면 안 된다. 3307 전체(120.7)는 더 넓어 방향제·데오도란트까지 든다.
    # 마스크팩은 셋으로 나눠 담는다. Epic·증권사가 '마스크팩'이라 부르는 건 330790 전체이고,
    # 관세청이 '마스크 팩'이라 이름 붙인 건 3307904000 하나다. 둘 다 필요하다 —
    # 잠정치(2주 빠름)는 330790 기준으로만 오고, 순수 마스크팩은 3307904000 로만 보인다.
    # 330790 안에서 마스크팩 비중이 52~77% 로 흔들려 서로 대체할 수 없다.
    {"hs": "330790", "label": "마스크팩 전체", "note": "Epic·증권사 기준 (330790) · 잠정치가 이 기준으로 온다"},
    {"hs": "3307904000", "label": "마스크팩",  "note": "관세청 품목명 '마스크 팩' (3307904000)"},
    {"hs": "3307909000", "label": "기타 조제화장품", "note": "330790 중 기타 (3307909000)"},
    ...
    {"hs": "3305901000", "label": "린스",    "note": "헤어린스 (3305901000)"},
    # 라면은 1902301010 이다(관세청 품목명 그대로 '라면').
    # 190230 은 '그 밖의 파스타' 라 라면 165.2 + 기타 6.3 + 3.4 가 섞여 174.9 가 된다.
    # 증권사 표의 라면 165 와 1902301010 이 정확히 일치한다.
    {"hs": "1902301010", "label": "라면",    "note": "라면 (1902301010) · 삼양식품·농심"},
...
def main():
    ...
    for ri in REGION_ITEMS:
        ...
        meta = REGION_META.get(ri["label"], {})
        out["items"].append({"hs": ri["hs"], "label": ri["label"], "note": ri["note"],
                             "region": True, "stock": meta.get("stock"), "natl": meta.get("natl"),
