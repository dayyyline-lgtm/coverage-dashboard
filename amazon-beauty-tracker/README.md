...
매일 1회, 아마존 **미국 탑100**과 **영국·독일·프랑스·이탈리아·스페인 탑50**에서 관심 브랜드
제품의 **순위·가격·평점·리뷰 수·하위 카테고리 BSR**을 수집해 `data/history.csv`에 누적하고,
전일 대비 변동을 **텔레그램**으로 보냅니다. 쌓인 CSV는 그대로 coverage dashboard의 데이터
소스로 씁니다.
...
```bash
python main.py --no-telegram --no-detail --markets US,UK
```
...
| `--no-detail` | 31~50위 상세 보강 생략 (빠른 확인용) |
...
| date | market | rank | asin | brand | title | price | currency | rating | reviews | sub_bsr_rank | sub_bsr_cat |
|---|---|---|---|---|---|---|---|---|---|---|---|

날짜×마켓×제품 단위로 한 행씩 쌓입니다. 같은 날 재실행하면 그 날 데이터는 덮어씁니다.
**관심 브랜드 제품만 저장**합니다 (전체 100개가 아님).

- `price`는 `currency` 컬럼의 통화 기준입니다. 마켓별 통화가 다르니 **합산·비교 전에 반드시
  환산**하세요.
- `sub_bsr_cat` / `sub_bsr_rank`는 하위 카테고리 순위(예: `Facial Toners & Astringents #1`)로,
  전체 Beauty 순위보다 민감한 판매 프록시입니다. 상세 조회가 일어난 31위 이하 제품에만
  채워집니다. **1~30위 제품에도 채우고 싶으면** `config.yaml`에서
  `detail.tracked_all_ranks: true`로 켜세요 (추적 브랜드 수만큼 요청이 늘어납니다).

`data/asin_cache.json`은 ASIN→제목 캐시입니다. 이게 있어야 이미 브랜드가 아닌 걸로 판명된
ASIN을 매일 다시 조회하지 않습니다. **지우지 마세요** (지우면 다음 실행이 느려질 뿐 오류는 아님).
