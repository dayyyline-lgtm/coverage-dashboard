...
def _is_earn_title(t):
    """기업설명회(IR) 제목이 '분기 실적발표'인가. 일반 IR(설명·Q&A·리뷰)과 구분.
       핵심: '설명/Q&A/질의응답/리뷰/이해도/간담' 은 실적을 '논의'하는 자리지
       실적 '발표(공시)' 자체가 아니다 → 제목에 '잠정'·'N분기 경영실적'이 있어도 ir.
       (실제 잠정실적 '숫자'는 영업(잠정)실적 공정공시가 classify RULES 로 따로 earn 처리된다.
        예: '2분기 경영실적 설명 및 질의응답', '경영실적 및 주요 관심사항 Q&A 진행',
            '잠정 경영 실적 설명 및 투자자 Q&A' — 전부 IR 설명회지 실적발표가 아니다.)
       - '설명/질의/Q&A/리뷰/이해도/간담' 포함 → 항상 ir
       - 순수 '실적발표' 또는 'N분기 + 경영실적/실적 발표' → earn """
    if any(k in t for k in ("설명", "질의", "Q&A", "Q & A", "리뷰", "이해도", "간담")):
        return False
    if "실적발표" in t:
        return True
