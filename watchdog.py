...
LIMITS = {
    # 영화(MOVIE·MOVIE_SCREENS)는 2026-09-21 수집을 멈췄다 — 여기서 빼지 않으면
    # 멈춘 그날부터 매일 '고장났다'고 알린다. 되살릴 땐 이 두 줄부터 되돌릴 것.
    ...
    "C2MODEL":       ("컴투스 모델 재료", 24 * 40),  # 분기 실적 + 월별 검색이라 한 달에 한 번 움직인다
    ...
    "DCGALL":        ("디시 글수", 30),        # 아침 수집에서 하루 1회
    "APPRANK":       ("앱스토어 게임순위", 30),
    "STORERANK":     ("스토어 판매·예약 순위", 30),   # 아침 수집에서 하루 1회 (PS·Steam·MS 스토어)
    "TWITCH":        ("트위치 시청자", 30),
    # 스냅샷 소스라 값이 안 바뀌면 파일을 안 건드린다 → asOf 도 안 움직인다.
    # 새벽에 순위가 굳는 날이 있어 시세(30h)보다 넉넉히 잡는다.
    "BUZZ":          ("지금 화제(구글·네이트·나무위키·디시)", 40),
    "BEAUTY":        ("올리브영 베스트", 40),   # 아침 수집에서 하루 1회
