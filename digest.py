...
def _arw(c):
    """등락 방향 — 한국식 색(상승 빨강·하락 파랑). 색+화살표 두 글자 대신 동그라미 하나로.
       부호(+/-)가 이미 방향을 말하므로 색만으로 충분하다."""
    return "⚪" if not c else ("🔴" if c > 0 else "🔵")
...
def build(html, alerts_only=False):
    ...
    # 관세청 잠정치 갱신일(1·11·21일) — 이날만 새 숫자가 나온다.
    # 자동 수집이 막혀 있어(무료 공개 품목에 화장품이 없다) 사람이 넣어 줘야 하는데,
    # 그냥 두면 잊는다. 그날 아침 레터가 먼저 말을 건다.
    if today.day in (1, 11, 21):
        pts.append("📦 <b>오늘 관세청 잠정치 갱신일</b> — 수출 표 보내주시면 반영합니다")
